    ...
    if config["audio_only"]:
        ...
        if format_id:
            ...
            cmd += [
                "-f",
                (
                    f"bestvideo[height={config['quality'].replace('p','')}][ext=mp4]+bestaudio[ext=m4a]/best[height={config['quality'].replace('p','')}][ext=mp4]/best[height={config['quality'].replace('p','')}]"
                    if "p" in config["quality"]
                    else config["quality"]
                ),
            ]
...
def download_video(
    ...
    while attempt < max_retries:
        try:
            ...
            # Let yt-dlp print directly to the console for robust logs and progress
            process = subprocess.Popen(
                cmd,
                # Remove stdout/stderr redirection for native yt-dlp progress/logs
            )
            process.wait()
        ...
        for url, data in playlists.items():
            ...
            if st.button(f"⬇️ Download Selected from {data['title']}", key=f"btn_{url}"):
                with st.status(
                    ...
                    filepaths = []
                    progress_bars = {
                        vid["id"]: st.progress(0.0, text=vid["title"])
                        for vid in selected_videos
                    }
                    download_status = {vid["id"]: 0.0 for vid in selected_videos}

                    def download_and_mark_vid(vid):
                        result = download_video(vid, output_dir)
                        download_status[vid["id"]] = (
                            1.0 if result["status"] == "Downloaded" else 0.0
                        )
                        return result
                    ...
                    with ThreadPoolExecutor(
                        ...
                        future_to_vid = {
                            executor.submit(download_and_mark_vid, vid): vid
                            for vid in selected_videos
                        }
                        for future in as_completed(future_to_vid):
                            vid = future_to_vid[future]
                            result = future.result()
                            progress_bars[vid["id"]].progress(
                                download_status[vid["id"]], text=vid["title"]
                            )
                            ...
                            if (
                                ...
                                candidate = (
                                    output_dir
                                    / f"{sanitize_filename(vid['title'])}.{ext}"
                                )
                                if candidate.exists():
                                    ...
                                    webm_candidate = (
                                        output_dir
                                        / f"{sanitize_filename(vid['title'])}.webm"
                                    )
...
# --- Single video parallel downloads ---
else:
    ...
    if st.button("⬇️ Download Videos", key="download_single_videos"):
        ...
        if not urls:
            ...
            with st.status("Downloading videos...", expanded=True) as status:
                ...
                results = []
                filepaths = []
                titles = [fetch_video_title(url) for url in urls]
                progress_bars = {
                    url: st.progress(0.0, text=title)
                    for url, title in zip(urls, titles)
                }
                # Use a dict to store download status for each url
                download_status = {url: 0.0 for url in urls}

                def download_and_mark(url, title):
                    result = download_video({"id": url, "title": title}, output_dir)
                    download_status[url] = (
                        1.0 if result["status"] == "Downloaded" else 0.0
                    )
                    return result
                ...
                with ThreadPoolExecutor(
                    ...
                    future_to_url = {
                        executor.submit(download_and_mark, url, title): (url, title)
                        for url, title in zip(urls, titles)
                    }
                    for future in as_completed(future_to_url):
                        url, title = future_to_url[future]
                        result = future.result()
                        progress_bars[url].progress(download_status[url], text=title)
                        st.write(f"{result['title']}: {result['status']}")
                        results.append(result)
                        if (
                            config["output_mode"] == "Download via Browser"
                            ...
                            if candidate.exists():
                                ...
                                webm_candidate = (
                                    output_dir / f"{sanitize_filename(title)}.webm"
                                )
                                if webm_candidate.exists():
                                    filepaths.append(webm_candidate)
                log_history(results)
